...
pub async fn get_file_content(uri_or_content: String) -> String {
    //If is a direct link
    if uri_or_content.starts_with("https://") {
        let content = download_file(uri_or_content).await;
        return content;
    }
    // If is a link generated by github it has the following format [name-file](link)
    // check if contains "https://" and "[", "]" and "(", ")" then it is a URI, download file
    else if uri_or_content.contains("https://")
